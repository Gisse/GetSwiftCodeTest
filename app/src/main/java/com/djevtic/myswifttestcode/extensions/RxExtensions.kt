...
fun <T> Observable<T>.computationToMain(): Observable<T> = compose { subscribeOn(Schedulers.computation()).observeOn(MAIN_SCHEDULER) }
...
fun <T> Maybe<T>.ioToMain(): Maybe<T> = compose { subscribeOn(WORKER_SCHEDULER).observeOn(MAIN_SCHEDULER) }
fun <T> Maybe<T>.workerToMain(): Maybe<T> = compose { subscribeAsync().observeMain() }
fun <T> Maybe<T>.computationToMain(): Maybe<T> = compose { subscribeOn(Schedulers.computation()).observeOn(MAIN_SCHEDULER) }
...
fun <T> Maybe<T>.onErrorResumeNextKt(f: ((t: Throwable) -> MaybeSource<T>)): Maybe<T> = onErrorResumeNext(Function { f.invoke(it) })

//Single

fun <T> Single<T>.ioToMain(): Single<T> = compose { subscribeOn(WORKER_SCHEDULER).observeOn(MAIN_SCHEDULER) }
fun <T> Single<T>.workerToMain(): Single<T> = compose { subscribeAsync().observeMain() }
fun <T> Single<T>.computationToMain(): Single<T> = compose { subscribeOn(Schedulers.computation()).observeOn(MAIN_SCHEDULER) }
...
fun <T> Flowable<T>.ioToMain(): Flowable<T> = compose { subscribeOn(WORKER_SCHEDULER).observeOn(MAIN_SCHEDULER) }
fun <T> Flowable<T>.workerToMain(): Flowable<T> = compose { subscribeAsync().observeMain() }
fun <T> Flowable<T>.computationToMain(): Flowable<T> = compose { subscribeOn(Schedulers.computation()).observeOn(MAIN_SCHEDULER) }
...
fun <T> Flowable<T>.onErrorResumeNextKt(f: ((t: Throwable) -> Publisher<T>)): Flowable<T> = onErrorResumeNext(Function { f.invoke(it) })
...
fun Completable.computationToMain(): Completable = compose { subscribeOn(Schedulers.computation()).observeOn(MAIN_SCHEDULER) }
